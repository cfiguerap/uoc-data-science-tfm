...
```{r}
data = read.csv("data/block_0.csv", stringsAsFactors = FALSE)
summary(data)
```
...
Podemos ver que los datos fecha deberían ser Date y energy deberían ser números.
...
```{r}
festivos = read.csv("data/uk_bank_holidays.csv", stringsAsFactors = FALSE)
headTail(festivos)
```
***
...
```{r}
meteo = read.csv("data/weather_hourly_darksky.csv", stringsAsFactors = FALSE)
headTail(meteo)
```
***
...
```{r}
#col_order <- c("fecha", "festivo", "consumo")
#data <- data[, col_order]
